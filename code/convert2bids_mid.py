...
Mat_Beh_dir='/ZPOOL/data/projects/istart-datapaper-test04/bids/sourcedata'
...
for ML_EvFile in ML_EvFiles_list:
    ...
    if lens.count(lens[0]) == len(lens):
        ...
        if not os.path.isdir('../../istart-datapaper-test-04/bids/%s/func'%(sub)):
            os.makedirs('../../istart-datapaper-test04/bids/%s/func'%(sub)) 
        df.to_csv('../../istart-datapaper-test04/bids/%s/func/%s_task-mid_run-%s_events.tsv'%(sub,sub,run),sep = '\t', index=False)
    else:
